...
export function ArticleEditor({
  ...
  // ── FIX: isMounted MUST be true before ANY Tiptap/ProseMirror code runs ──
  ...
  // ── FIX: callbacks stable ref to avoid stale closures in useMemo ──
  const slashCallbacksRef = useRef({
    ...
    onUpdate:  (query: string) => { setSlashItems(getSlashItems(query)); },
    onClose:   () => setSlashMenuOpen(false),
    onKeyDown: (event: KeyboardEvent) => slashRef.current?.onKeyDown({ event }) ?? false,
    getItems:  getSlashItems,
  });

  // ── FIX: Build extensions only after mount (client-side only) ──────────
  // This prevents the 'localsInner' ProseMirror crash that occurs when
  // Tiptap extensions (especially @tiptap/suggestion plugins) are
  // instantiated during SSR / pre-render before the DOM is available.
  const extensions = useMemo(() => {
    if (!isMounted) return [];  // ← KEY FIX: no extensions before mount
    return [
  ...
  }, [isMounted]); // rebuilds once when isMounted flips to true
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      // Manual slash detection fallback
      const { state } = editor;
      ...
      if (match) {
        ...
        if (dom.node instanceof Element) {
          const rect = (dom.node as Element).getBoundingClientRect();
  ...
  // Mount flag — must be first effect
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (mode === 'create' && !articleId) {
        const created = await apiClient.articles.create(payload);
  ...
  if (!isMounted) {
    // ── FIX: Show a loading skeleton instead of rendering editor
    // before hydration. This also prevents the localsInner crash.
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-6">
        <div className="text-center">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[var(--border)] border-t-[var(--text-accent)]" />
  ...
  if (mode === 'edit' && isLoadingArticle) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-6">
        <div className="text-center">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-[var(--border)] border-t-[var(--text-accent)]" />
          <p className="mt-4 text-[var(--text-secondary)]">Chargement de l'article…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-full min-h-screen bg-[var(--bg-primary)]">
      <div className={cn('flex-1 flex flex-col transition-all', sidePanel ? 'mr-80' : '')}>
        <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg-secondary)] px-6 py-4">
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">
              {mode === 'create' ? 'Nouvel article' : 'Éditeur'}
            </h1>
          </div>
        ...
        <div className="flex items-center justify-between border-t border-[var(--border)] bg-[var(--bg-secondary)] px-6 py-4">
          <div className="text-sm text-[var(--text-secondary)]">{charCount} caractères</div>
          <div className="flex gap-2">
            <button
              onClick={() => setSidePanel(sidePanel === 'meta' ? null : 'meta')}
              className="flex items-center gap-2 rounded px-4 py-2 hover:bg-[var(--border)]"
            >
              <Settings size={18} />
              Métadonnées
            </button>
            {mode === 'edit' && (
              <>
                ...
                <button
                  ...
                  className="rounded bg-[var(--text-accent)] px-4 py-2 text-white disabled:opacity-50"
                >
                  {isSubmitting ? 'En cours…' : published ? 'Publié' : 'Publier'}
                </button>
                <button
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded px-4 py-2 text-red-500 hover:bg-red-500/10"
                >
                  <X size={18} />
                  Archiver
                </button>
        ...
        <div className="fixed right-0 top-0 bottom-0 w-80 bg-[var(--bg-primary)] border-l border-[var(--border)] overflow-y-auto z-10">
          <div className="flex items-center justify-between border-b border-[var(--border)] p-4">
            <h2 className="font-semibold">
              {sidePanel === 'meta' ? 'Métadonnées' : 'Historique'}
            </h2>
            <button
          ...
          <div className="p-4">
            {sidePanel === 'meta' ? (
              ...
              <>
                {articleId && <ArticleVersionHistory articleId={articleId} />}
              </>
            )}
          </div>
        </div>
      ...
      {showImageUpload && (
        <EditorImageUpload onClose={() => setShowImageUpload(false)} />
      )}
    </div>
  );
}
